...
class Game:
    ...
    def is_valid_move(self, coords: CoordPair) -> bool:
        ...
        unit_d = self.get(coords.dst)


            
    # Rule 1: The destination must be free
        if unit_d is not None:
            print("Destination is not free")
            return False

    # Rule 2: Check if units are engaged in combat
        for adj_coord in coords.src.iter_adjacent():
            adj_unit = self.get(adj_coord)
            if adj_unit is not None and adj_unit.player != unit_s.player:
                print("Units are engaged in combat")
    ...
    # Rule 3: Movement directions based on unit type and player
        
        if unit_s.type in [UnitType.AI, UnitType.Firewall, UnitType.Program]:
            # Attacker's AI, Firewall, and Program can only move up or left
                if unit_s.player == Player.Attacker:
                    if coords.src.row < coords.dst.row or coords.src.col < coords.dst.col:
                    # print("valid move direction for attacker's AI, Firewall,or Program")
                        return False
                
                elif unit_s.player == Player.Defender:
            # else:
                    if coords.dst.row == coords.src.row and coords.dst.col == coords.src.col:
                       return False
    ...
    def perform_move(self, coords : CoordPair) -> Tuple[bool,str]:
        """Validate and perform a move expressed as a CoordPair."""
         ...
         #Check if the distance between source and destination is greater than 2
        row_diff = abs(coords.dst.row - coords.src.row)
        col_diff = abs(coords.dst.col - coords.src.col)

        if row_diff <= 1 and col_diff <= 1:
    # The source and destination are within 1 row or 1 column of each other
           pass
        else:
            print("Invalid Move: Source and destination are more than 1 positions apart.")
            return False
        ...
        if row_diff == 0 and col_diff <= 1:
           pass
            ...
            print("Invalid Move: Diagonal moves are.")
            return False
        ...
        if self.is_valid_move(coords):
            ...
            return (True, "Valid move performed")
            ...
            return (True, "Valid Attack performed")
            ...
            return (True, "Valid Repair performed")
        elif self.can_self_destruct(coords):
            print("Valid self destruct")
            self.mod_health(coords.src,-9)
            for surrounding_coord in coords.src.iter_range(1):
                self.mod_health(surrounding_coord,-2)
            return (True, "valid self destruct performed")
        
        elif self.can_move_program_firewall_ai(coords) :
            """Check if the move for Program, Firewall and AI is valid"""
            self.set(coords.dst, self.get(coords.src))
            self.set(coords.src, None)
            return (True, "Valid move for Program, Firewall, or AI performed")
        
        return (False,"invalid move")
        
        

    def can_move_program_firewall_ai(self, coords: CoordPair) -> bool:
        """Check if the move is valid for Program, Firewall, or AI units."""
        if not self.is_valid_coord(coords.src) or not self.is_valid_coord(coords.dst):
            return False
        unit_s = self.get(coords.src)
    
    # Program,Firewall,and AI units to move in left or up direction
        if unit_s.type in [UnitType.Program, UnitType.Firewall, UnitType.AI]:
